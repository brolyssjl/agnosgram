...
| `link.outside-project` | a Markdown link target is absolute, normalizes outside the project root, or resolves through a symlink - never probed for existence, so it cannot be used to test whether an arbitrary path exists |
