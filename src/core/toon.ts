 ...
 *
 * Ported from Gate (`gate/src/serialize/toon.ts`), which owns the design: the
 * token win comes entirely from rendering uniform arrays of flat objects as a
 * header + comma rows. Other shapes fall back to a readable, indented,
 * YAML-like form. This is an encoder only - Agnosgram never parses TOON back
 * and never stores it; `.agnosgram/` stays plain Markdown + YAML (DEC-0001).
 */
