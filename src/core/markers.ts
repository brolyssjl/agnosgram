/**
 * Managed-block injection. Agnosgram owns only the text between its markers in an
 * agent config file; everything else the user wrote is preserved byte-for-byte.
 * Rewriting is idempotent: running it twice yields an identical file.
 */

export const START_MARKER = "<!-- agnosgram:start -->";
export const END_MARKER = "<!-- agnosgram:end -->";
